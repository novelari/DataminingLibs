...
template <typename T>
__global__ void host_kernel(GpuRf<T>::GpuParams* params,
                            GpuRfStatic::GpuRfKernelId type) {
  switch (type) {
    case GpuRfStatic::kSetupKernel:
      GpuRf<T>::gpurf_setup_kernel(params);
      break;
    case GpuRfStatic::kInitTreeBatch:
      GpuRf<T>::gpurf_initialize_tree_batch(params);
      break;
    case GpuRfStatic::kFindSplit:
      GpuRf<T>::gpurf_find_split(params);
      break;
    case GpuRfStatic::kPerformSplit:
      GpuRf<T>::gpurf_perform_split(params);
      break;
    case GpuRfStatic::kPredict:
      GpuRf<T>::gpurf_predict(params);
      break;
    case GpuRfStatic::kOobEstimate:
      GpuRf<T>::gpurf_oob_estimate(params);
      break;
    case GpuRfStatic::kFeatureImp:
      GpuRf<T>::gpurf_feature_importance(params);
      break;
    default:
      break;
  }
...
sp<lib_models::MlModel> GpuRf<T>::Fit(sp<lib_data::MlDataFrame<T>> data,
  ...
  gpuDTE_NodeHeader_Classify tmp_node;
  col_array<gpuDTE_NodeHeader_Classify> tree_nodes;
  ...
  col_array<col_array<gpuDTE_NodeHeader_Train>> node_cache(
      2, col_array<gpuDTE_NodeHeader_Train>());
    ...
    do {
      ...
      do {
        ...
        host_kernel<T><<<nodes_launched, nr_threads_in_block>>>(
            gpu_params, GpuRfStatic::kFindSplit);
        device.SynchronizeDevice(0);

        host_kernel<T><<<nodes_launched, nr_threads_in_block>>>(
            gpu_params, GpuRfStatic::kPerformSplit);
        device.SynchronizeDevice(0);
  ...
  // Run prediction process
  {
    ...
    while (total_threads > 0) {
      ...
      host_kernel<T><<<launch_threads / block_size, block_size>>>(
          gpu_params, GpuRfStatic::kPredict);
      device.SynchronizeDevice(0);
...
template <typename T>
void GpuRf<T>::AllocateFit(sp<MlAlgorithmParams> params, GpuParams* gpu_params,
  ...
  for (int i = 0; i < 3; ++i)
    lib_gpu::GpuDeviceCuda::AllocateManagedMemory(
        ...
        sizeof(gpuDTE_NodeHeader_Train) * nr_nodes_per_batch);

  // Tree header buffer
  lib_gpu::GpuDeviceCuda::AllocateManagedMemory(
      &gpu_params->node_tmp_buffer,
      sizeof(gpuDTE_TmpNodeValues) * nr_nodes_per_batch);
  lib_gpu::GpuDeviceCuda::AllocateManagedMemory(
  ...
  lib_gpu::GpuDeviceCuda::AllocateManagedMemory(&gpu_params->dataset_info,
                                                sizeof(gpuDTE_DatasetInfo));
  lib_gpu::GpuDeviceCuda::AllocateManagedMemory(&gpu_params->static_info,
                                                sizeof(gpuDTE_StaticInfo));
  lib_gpu::GpuDeviceCuda::AllocateManagedMemory(&gpu_params->iteration_info,
                                                sizeof(gpuDTE_IterationInfo));
...
void GpuRf<T>::AllocatePredict(sp<MlAlgorithmParams> params,
  ...
  auto& node_headers =
      model->Get<col_array<gpuDTE_NodeHeader_Classify>>(ModelsLib::kNodeArray);
  auto& prob_data = model->Get<col_array<T>>(ModelsLib::kProbArray);

  lib_gpu::GpuDeviceCuda::AllocateManagedMemory(
      &gpu_params->node_buffers_classify,
      sizeof(gpuDTE_NodeHeader_Classify) * int(node_headers.size()));
...
template <typename T>
void GpuRf<T>::FreeParams(GpuParams** dev_ptr) {
  if (*dev_ptr) {
    (*dev_ptr)->finalize();
...
template <typename T>
GpuRf<T>::GpuParams* GpuRf<T>::CreateParams() {
  GpuParams* dev_ptr = nullptr;
  lib_gpu::GpuDeviceCuda::AllocateManagedMemory(&dev_ptr, sizeof(GpuParams));
  dev_ptr->init();
  return dev_ptr;
}
...
template <typename T>
__device__ void GpuRf<T>::gpurf_setup_kernel(GpuParams* params) {
...
template <typename T>
__device__ void GpuRf<T>::gpurf_initialize_tree_batch(GpuParams* params) {
  ...
  if (threadIdx.x == 0) {
    GpuDte<T>::gpuDTE_NodeHeader_Train root;
    root.parent_id = -2;
    root.attribute = -2;
    root.split_point = -2;
    root.trackinid = GpuDte<T>::AtomicAdd(&params->node_cursors[node_id_], 1);
...
template <typename T>
__device__ void GpuRf<T>::gpurf_find_split(GpuParams* params) {
  __shared__ T s_dynamic_shared[40];
  __shared__ unsigned int s_histograms[1024];
  __shared__ unsigned int s_offsets[256];
  __shared__ GpuDte<T>::gpuDTE_NodeHeader_Train s_tree_node;
  __shared__ GpuDte<T>::gpuDTE_TmpNodeValues s_tmp_node;
...
template <typename T>
__device__ void GpuRf<T>::gpurf_perform_split(GpuParams* params) {
  extern __shared__ int s_node_counts[];

  GpuDte<T>::gpudte_perform_split(
...
template <typename T>
__device__ void GpuRf<T>::gpurf_predict(GpuParams* params) {
...
template <typename T>
__device__ void GpuRf<T>::gpurf_oob_estimate(GpuParams* params) {
  if (threadIdx.x + blockIdx.x * blockDim.x >=
      params->iteration_info->threads_launched)
    return;
  ...
  float dataPoint;
  GpuDte<T>::gpuDTE_NodeHeader_Classify tree_node =
...
template <typename T>
__device__ void GpuRf<T>::gpurf_feature_importance(GpuParams* params) {
  if (threadIdx.x + blockIdx.x * blockDim.x >=
      params->iteration_info->threads_launched)
    return;
  ...
  while (tree_node.child_count != 0) {
    ...
    dataPoint = GpuDte<T>::get_data_point(tree_node.attribute, instance,
                                          params->dataset_info->nr_instances,
                                          params->dataset);
    if (attribute_type > max_nominal_ ||
        params->dataset_info->data_type == type_regression_) {
...
__device__ void GpuRf<T>::radix_sort_on_attribute(
    GpuParams* params, GpuDte<T>::gpuDTE_NodeHeader_Train& node,
    GpuDte<T>::gpuDTE_TmpNodeValues& tmp_node, unsigned s_histograms[1024],
  ...
  for (int i = threadIdx.x; i < node.node_index_count; i += blockDim.x) {
    ...
    GpuDte<T>::AtomicAdd(&s_histograms[*dataVal], unsigned int(1));
    GpuDte<T>::AtomicAdd(&s_histograms[256 + (*(dataVal + 1))],
                         unsigned int(1));
    GpuDte<T>::AtomicAdd(&s_histograms[512 + (*(dataVal + 2))],
                         unsigned int(1));
    GpuDte<T>::AtomicAdd(&s_histograms[768 + (*(dataVal + 3))],
                         unsigned int(1));
  }
    ...
    if (j != 3) {
      ...
      if (performPass) {
        ...
        for (int i = 0; i < node.node_index_count; i += blockDim.x) {
          ...
          if (!skip) {
            GpuDte<T>::AtomicAdd(&curCount[radix], unsigned int(1));
...
template <typename T>
__device__ T GpuRf<T>::eval_numeric_attribute(GpuParams* params,
                                              gpuDTE_NodeHeader_Train& node,
                                              gpuDTE_TmpNodeValues& tmp_node,
                                              T* curr_dist, int att_type,
                                              unsigned* s_histograms,
                                              unsigned* s_offsets) {
  ...
  for (int i = threadIdx.x; i < numInds; i += blockDim.x) {
    inst = params->indices_buffer[params->iteration_info->tick_tock ? 0 : 1]
                                 [nodeIndStart + i];
    val = GpuDte<T>::get_data_point(tmp_node.tmp_attribute, inst,
                                    params->dataset_info->nr_instances,
                                    params->dataset);
...
template <typename T>
__device__ T GpuRf<T>::variance_calculation(GpuParams* params,
                                            gpuDTE_NodeHeader_Train& node,
                                            gpuDTE_TmpNodeValues& tmp_node,
...
template <typename T>
void GpuRf<T>::GpuParams::finalize() {
