...
#define flt_max 3.402823466e+38F
 ...
 private:
  void AllocateFit(sp<MlAlgorithmParams> params,
                   GpuRfStatic::GpuParams<T> *gpu_params,
                   sp<lib_data::MlDataFrame<T>> data);
  void AllocatePredict(sp<MlAlgorithmParams> params,
                       GpuRfStatic::GpuParams<T> *gpu_params,
                       sp<lib_data::MlDataFrame<T>> data,
                       sp<lib_models::MlModel> model);

  void FreeParams(GpuRfStatic::GpuParams<T> **dev_ptr);
  GpuRfStatic::GpuParams<T> *CreateParams();
 ...
 public:
  ...
  __device__ T evaluate_nominal_attribute(
      ...
      GpuRfStatic::gpuDTE_TmpNodeValues<T> &tmp_node, T *curr_dist,
  ...
  __device__ void gpudte_perform_split(
      ...
      GpuRfStatic::gpuDTE_IterationInfo &iteration_info,
  ...
  __device__ void gpurf_find_split(GpuRfStatic::GpuParams<T> *params);
  __device__ void gpurf_perform_split(GpuRfStatic::GpuParams<T> *params);
  __device__ void gpurf_predict(GpuRfStatic::GpuParams<T> *params);
  __device__ void gpurf_oob_estimate(GpuRfStatic::GpuParams<T> *params);
  __device__ void gpurf_feature_importance(GpuRfStatic::GpuParams<T> *params);
  ...
  __device__ T
  variance_calculation(GpuRfStatic::GpuParams<T> *params,
                       GpuRfStatic::gpuDTE_NodeHeader_Train<T> &node,
                       GpuRfStatic::gpuDTE_TmpNodeValues<T> &tmp_node,
